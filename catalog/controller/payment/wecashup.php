...
class ControllerPaymentWecashup extends Controller {
	protected function index() {
		...
		if ($this->config->get('wecashup_test') == 'live') {
			$this->data['action'] = 'https://live.wecashup.com/gateway/service/vspform-register.vsp';
		} elseif ($this->config->get('wecashup_test') == 'test') {
			$this->data['action'] = 'https://test.wecashup.com/gateway/service/vspform-register.vsp';		
		} elseif ($this->config->get('wecashup_test') == 'sim') {
			$this->data['action'] = 'https://test.wecashup.com/simulator/vspformgateway.asp';
		}

		$vendor = $this->config->get('wecashup_vendor');
		$password = $this->config->get('wecashup_password');		
		...
		$data['SuccessURL'] = str_replace('&amp;', '&', $this->url->link('payment/wecashup/success', 'order_id=' . $this->session->data['order_id']));
		...
		if (!$this->config->get('wecashup_transaction')) {
			$data['ApplyAVSCV2'] = '0';
		}

		$data['Apply3DSecure'] = '0';

		$this->data['transaction'] = $this->config->get('wecashup_transaction');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/wecashup.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/wecashup.tpl';
		} else {
			$this->template = 'default/template/payment/wecashup.tpl';
		}	
	...
	public function success() {
		if (isset($this->request->get['crypt'])) {
			...
			if ($data && is_array($data)) {
				...
				if ($data['Status'] == 'OK') {
					$this->model_checkout_order->update($this->request->get['order_id'], $this->config->get('wecashup_order_status_id'), $message, false);
				} else {
